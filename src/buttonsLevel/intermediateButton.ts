...
import { paginationIntermediate } from "./paginationIntermediate";

export async function intermediateHandler() {
  const mainContainer = document.querySelector(".main") as HTMLElement;
  const responseWords = await Words.getWords("2", "0");
  ...
  <div class="word-wrapper">
    ...
    `;
  audioBtnWordPlay();
  paginationIntermediate(30, 1);
}

export const intermediateLevel = () => {
  const intermediateBtn = document.querySelector(
    ".intermediate"
  ) as HTMLElement;

  intermediateBtn.addEventListener("click", intermediateHandler);
};
