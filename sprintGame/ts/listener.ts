...
export function getGameListener(event: Event) {
  ...
  const separatorTrueAnswer = <HTMLElement>(
    document.querySelector(".separator-true-answer")
  );
  const separatorFalseAnswer = <HTMLElement>(
    document.querySelector(".separator-false-answer")
  );
  ...
  if ((event.target as HTMLElement)?.dataset.btn === flagStartGame.value) {
    ...
    if (countflag < 2) {
      ...
      countflag = -1;
      if (valueCounterSprint && Number(valueCounterSprint.textContent) <= 60) {
...
function getGameListenerKeydown(event: KeyboardEvent) {
  ...
  if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
    if (
      (event.key === "ArrowRight" && flagStartGame.value === "true") ||
      (event.key === "ArrowLeft" && flagStartGame.value === "false")
    ) {
      console.log("true");
      playAudioCool();
      if (flagStartGame.value === "true") {
    ...
    if (
      (event.key === "ArrowLeft" && flagStartGame.value === "true") ||
      (event.key === "ArrowRight" && flagStartGame.value === "false")
    ) {
      console.log("false");
      playAudioError();
      separatorFalseAnswer.style.display = "inline-block";
